...
// CI job 6 step "Security test cases S-01 to S-47 (npm run test:security)".
//
...
// (slice 13-MFA extends it to S-47 -- see the block at the end of testMatch),
...
module.exports = {
  ...
  testMatch: [
    ...
    // S-45 challenge-token single use, S-46 the CONCURRENT versions of S-36
    // and S-45 (review finding I-2 — both guarantees were sequential-only).
    ...
    '<rootDir>/test/integration/mfa.spec.ts', // S-36..S-42, S-45, S-46
    // S-43 (forced-change user blocked everywhere else), S-44 (password
    // change revokes other sessions), S-47 (FORCE_PASSWORD_CHANGE_ENABLED
    // gates the forcing, default off).
