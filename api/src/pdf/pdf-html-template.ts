...
<style>
  ...
  /* NO text-transform here. The caption comes from signatureBlockLabel() and is
     the paper form's own wording ("Maintenance Performed By", "Verified By
     (Workshop Team Leader)"). Chromium bakes text-transform into the rendered
     glyphs, so uppercasing it changes what pdf-parse extracts — which broke
     four assertions in records-pdf.spec.ts that pin those exact captions.
     Cosmetic styling must not alter the text of a controlled record. */
  .p-sign .lbl { font-size: 8px; font-weight: 700; color: #444; }
