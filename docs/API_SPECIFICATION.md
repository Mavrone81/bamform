...
| `/errors/archived-record-dependency` | 409 | The write would change what an already-archived record PRINTS (INV-09). Raised by `PATCH /asset-documents/{id}` (`machineNumber` → document title) and `PATCH /assets/{id}` (`code` → machine code, `description`). `detail` names the blocking records and quotes the before/after |
